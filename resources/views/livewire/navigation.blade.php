...
<nav class="bg-gray-800" x-data="{open: false}">
    <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
      <div class="relative flex items-center justify-between h-16">
        ...
        {{-- con la directiva blade @auth se verifica si el usuario inicio sesion --}}
        @auth
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
          ...
          <!-- Profile dropdown -->
          <div class="ml-3 relative" x-data="{open: false}">{{-- x-data es una propiedad de alpine para tratarlo como componente solo --}}
            <div>
                {{-- el x-on:click="" es un evento de js pero con alpine --}}
            ...
            -->
            {{-- el x-show es para usar el valor de la variable open creada en el x-data --}}
            <div x-show="open" x-on:click.away="open = false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
              ...
              {{-- con esta directiva de blade solo muestra la parte de administrador si se tiene el permiso amdin.home --}}
              @can('admin.home')
                <a href="{{route('admin.home')}}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-0">Panel administrador</a>              
              @endcan

              {{-- este form autentifica la salida del logueo --}}
